...
var EXPORTED_SYMBOLS = ["jsSHA", "jsOTP"];
var jsOTP;
var jsSHA;

(function () {
  var Totp = function(expiry, length) {
    // pass in the secret, code dom element, ticker dom element
    this.expiry = (typeof expiry !== "undefined") ? expiry : 30;
    this.length = (typeof length !== "undefined") ? length : 6;
  ...
  Totp.prototype.getOtp = function(secret, now) {
    now = (typeof now !== "undefined") ? now : new Date().getTime();
  
    var key = this.base32tohex(secret);
    var epoch = Math.round(now / 1000.0);
    var time = this.leftpad(
    ...
    var otp = (
      this.hex2dec(hmac.substr(offset * 2, 8)) &
      this.hex2dec("7fffffff")
    ) + "";
  ...
  Hotp.prototype.getOtp = function(key, counter) {
    ...
    shaObj.update(
      this.uintToString(
        new Uint8Array(this.intToBytes(counter))
      )
    );
